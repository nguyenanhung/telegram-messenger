...
class TelegramMessenger
{
    ...
    const METHOD_FORWARD_MESSAGE = '/forwardMessage';
    const METHOD_FORWARD_MESSAGES = '/forwardMessages';
    const METHOD_COPY_MESSAGE = '/copyMessage';
    const METHOD_COPY_MESSAGES = '/copyMessages';

    /** @var array|null SDK Config */
    protected $sdkConfig;

    /** @var null|string|int Unique identifier for the target chat or username of the target channel (in the format @channelusername) */
    protected $chatId;

    /** @var null|string|int Unique identifier for the chat where the original message was sent (or channel username in the format @channelusername) */
    protected $fromChatId;

    /** @var null|string Text of the message to be sent, 1-4096 characters after entities parsing */
    protected $message;
    ...
    public function sendMessage()
    {
        ...
        if (isset($res->ok, $res->result) && ($res->ok === true)) {
            ...
            $responseMsg = self::_CLASS_NAME_ . ' -> ' . self::METHOD_SEND_MESSAGE . ' Success with Params -> ' . json_encode(
                    $params
                );
            if (function_exists('log_message')) {
    ...
    public function sendPhoto(): bool
    {
        ...
        if (isset($res->ok, $res->result) && ($res->ok === true)) {
            // Gửi tin nhắn thành công
            $responseMsg = self::_CLASS_NAME_ . ' -> ' . self::METHOD_SEND_PHOTO . ' Success with Params -> ' . json_encode(
                    $params
                );
            if (function_exists('log_message')) {
                log_message('debug', $responseMsg);
    ...
    public function sendAudio(): bool
    {
        ...
        if (isset($res->ok, $res->result) && ($res->ok === true)) {
            // Gửi tin nhắn thành công
            $responseMsg = self::_CLASS_NAME_ . ' -> ' . self::METHOD_SEND_AUDIO . ' Success with Params -> ' . json_encode(
                    $params
                );
            if (function_exists('log_message')) {
                log_message('debug', $responseMsg);
    ...
    public function sendVideo(): bool
    {
        ...
        if (isset($res->ok, $res->result) && ($res->ok === true)) {
            // Gửi tin nhắn thành công
            $responseMsg = self::_CLASS_NAME_ . ' -> ' . self::METHOD_SEND_VIDEO . ' Success with Params -> ' . json_encode(
                    $params
                );
            if (function_exists('log_message')) {
                log_message('debug', $responseMsg);
    ...
    public function sendDocument(): bool
    {
        ...
        if (isset($res->ok, $res->result) && ($res->ok === true)) {
            // Gửi tin nhắn thành công
            $responseMsg = self::_CLASS_NAME_ . ' -> ' . self::METHOD_SEND_DOCUMENT . ' Success with Params -> ' . json_encode(
                    $params
                );
            if (function_exists('log_message')) {
                log_message('debug', $responseMsg);
